...
namespace ProjectEuler.Helpers
{
    ...
    class PrimeNumbers
    {
        ...
        public List<Int64> CalculatePrimeFactor(Int64 _number)
        {            
            return CalculatePrimeFactor(_number, 2);
        }
        ...
        /// <returns></returns>
        private List<Int64> CalculatePrimeFactor(long _number, Int64 _currentFactor = 2)
        {
            List<Int64> result;
            ...
            foreach (var factor in Numbers.Where(p => p >= _currentFactor && p <= _number))
            {
                ...
                if ((_number % factor) == 0)
                {
                    ...
                    else
                    {
                        // continue
                        result = new() { factor };
                        var sub = CalculatePrimeFactor(_number / factor, factor).ToList();
